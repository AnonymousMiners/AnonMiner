...
相较于市面上通过Gost隧道简单实现的TLS或AES加密程序，**无名矿工**是一款**集反抽水、超频、主被控管理、主动对抗策略、伪装与加密、修改流量特征、调整流量比例模型**于一身的综合性自研**GPU挖矿程序**，本程序匿名程度远超前者。
...
|标签|说明  |
|--|--|
|**`返还开发费`**  |  劫持所有与钱包地址不匹配的连接（包括挖矿软件），并重定向到你的地址。 **默认处于关闭状态。** |
